...
class SimpleRobotWrapper:
    ...
    def set_position(self, x: float, y: float, z: float, wait: bool = True) -> None:
        # print("set pos", x, y, z)
        self.robot.movel(
        ...
        # pose.set_pos(np.array([x, y, z]))
        # self.robot.servoc(pose, acc=self.ACC, vel=self.VEL, threshold=0.01, wait=wait)
...
class BaseEnv:
    ...
    def get_frames(self) -> Tuple[np.ndarray, np.ndarray]:
        if self.cfg.with_camera:
            ...
            color_image = np.asanyarray(color_frame.get_data())[110:-40, 50:-70]
            depth_frame = frames.get_depth_frame()
            depth_image = np.asanyarray(depth_frame.get_data())[120:-80, 150:-190]
    ...
    def obs_space(self) -> Dict[str, embodied.Space]:
        return {
            ...
            "joint_positions": embodied.Space(np.float32, (6,)),
...
class UR5PickPlace(BaseEnv):  # GraspRewardEnv
    # x <--|
    #      |
    #      v
    #      y

    LEFT_XY_MIN = (-0.0799, -0.66)
    LEFT_XY_MAX = (-0.013, -0.38)
    LEFT_SAFE_XY_MIN = (-0.0799 + X_ADJ, -0.66 + Y_ADJ)
    LEFT_SAFE_XY_MAX = (-0.013 - X_ADJ, -0.38 - Y_ADJ)
    RIGHT_XY_MIN = (-0.360, -0.66)
    RIGHT_XY_MAX = (-0.280, -0.38)
    RIGHT_SAFE_XY_MIN = (-0.360 + X_ADJ, -0.66 + Y_ADJ)
    RIGHT_SAFE_XY_MAX = (-0.280 - X_ADJ, -0.38 - Y_ADJ)

    Z_TABLE = -0.025 + DEBUG_DELTA
    Z_HOVER = 0.12 + DEBUG_DELTA
    Z_RESET = 0.18 + DEBUG_DELTA
    COUNTDOWN_STEPS = 5

    def random_xy_grid(self, side: Side) -> Tuple[float, float]:
        if side == Side.LEFT:
            x = np.random.uniform(self.LEFT_SAFE_XY_MIN[0], self.LEFT_SAFE_XY_MAX[0])
            y = np.random.uniform(self.LEFT_SAFE_XY_MIN[1], self.LEFT_SAFE_XY_MAX[1])
        elif side == Side.RIGHT:
            x = np.random.uniform(self.RIGHT_SAFE_XY_MIN[0], self.RIGHT_SAFE_XY_MAX[0])
            y = np.random.uniform(self.RIGHT_SAFE_XY_MIN[1], self.RIGHT_SAFE_XY_MAX[1])

        x = np.round(x / self.cfg.max_delta_m) * self.cfg.max_delta_m
        y = np.round(y / self.cfg.max_delta_m) * self.cfg.max_delta_m

        if side == Side.LEFT:
            x = np.clip(x, self.LEFT_SAFE_XY_MIN[0], self.LEFT_SAFE_XY_MAX[0])
            y = np.clip(y, self.LEFT_SAFE_XY_MIN[1], self.LEFT_SAFE_XY_MAX[1])
        elif side == Side.RIGHT:
            x = np.clip(x, self.RIGHT_SAFE_XY_MIN[0], self.RIGHT_SAFE_XY_MAX[0])
            y = np.clip(y, self.RIGHT_SAFE_XY_MIN[1], self.RIGHT_SAFE_XY_MAX[1])
        return x, y

    def __init__(self, real: bool = True, cfg: Optional[EnvConfig] = None) -> None:
        if cfg is None:
        ...
        if real:
            ...
            self._arm.set_z(self.Z_RESET)
            x, y = self.random_xy_grid(self._ball_side)
            self._arm.set_position(x, y, self.Z_RESET)
            self._reset()
    ...
    def current_bounds(self) -> Tuple[np.ndarray, np.ndarray, float]:
        ...
        if side == Side.LEFT:
            if self.grasped_object:
                return (
                    np.array(self.LEFT_SAFE_XY_MIN),
                    np.array(self.RIGHT_SAFE_XY_MAX),
                    self.Z_HOVER,
                )
            else:
                return (
            ...
            if self.grasped_object:
                return (
                    np.array(self.RIGHT_SAFE_XY_MIN),
                    np.array(self.RIGHT_SAFE_XY_MAX),
                    self.Z_HOVER,
                )
            else:
    ...
    def compute_arm_position(self, control_action: np.ndarray) -> np.ndarray:
        ...
        if self.grasped_object:
            side = self.arm_side()
            if side == Side.LEFT and desired_pose[0] + 0.005 < target_pose[0]:
                target_pose[:2] = np.clip(
                    target_pose[:2], self.RIGHT_XY_MIN, self.RIGHT_XY_MAX
                )
            if side == Side.RIGHT and desired_pose[0] - 0.005 > target_pose[0]:
                target_pose[:2] = np.clip(
                    target_pose[:2], self.LEFT_XY_MIN, self.LEFT_XY_MAX
                )
    ...
    def step(self, action: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if action["action"] < 4:
            ...
            self._arm.set_position(xyzrpy[0], xyzrpy[1], xyzrpy[2])
    ...
    def _reset(self) -> Dict[str, Any]:
        if self.grasped_object:
            ...
            x, y = self.random_xy_grid(self._ball_side)
            self._arm.set_position(x, y, self.Z_HOVER)
        ...
        for _ in range(3):
            for corner_x, corner_y in ([0, 0], [0, 1], [1, 1], [1, 0]):
                if corner_x == 0:
                    x = xyz_min[0]
                else:
                    x = xyz_max[0]
                ...
                self._arm.set_position(x, y, self.Z_TABLE)
    ...
    def get_reward(self, curr_obs: Dict[str, Any]) -> float:
        ...
        if not grasped_old and grasped_new:  # grasped it
            ...
            if arm_side == Side.LEFT:
                cart_pos[:2] = np.clip(
                    cart_pos[:2], self.LEFT_SAFE_XY_MIN, self.LEFT_SAFE_XY_MAX
                )
            else:
                cart_pos[:2] = np.clip(
                    cart_pos[:2], self.RIGHT_SAFE_XY_MIN, self.RIGHT_SAFE_XY_MAX
                )
            cart_pos[2] = self.Z_HOVER
            self._arm.set_position(cart_pos[0], cart_pos[1], cart_pos[2])
            return 1

        if grasped_old and not grasped_new:  # dropped it
            assert arm_side == self.grasped_bin
            rew = -1
            self._arm.set_z(self.Z_TABLE)
            self.grasped_bin = Side.OTHER
            return rew
...
class SpaceMouseAgent:
    ...
    def act(self, obs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        gripper_open = obs["gripper_pos"] < 0.01
...
def main(env_config: EnvConfig) -> None:
    ...
    env = UR5PickPlace(cfg=env_config)
...
if __name__ == "__main__":
    # robot = SimpleRobotWrapper()
    # # robot.robot.movel((0, 0, 0.01, 0, 0, 0), relative=True, wait=True)
    # o = np.array([[0, -1, 0], [-1, 0, 0], [0, 0, -1]])
    # robot.robot.set_orientation(o, acc=0.5, vel=2.5)
    # robot.set_gripper_position(0)
    # print(f"Read Gripper Pos: {robot.get_gripper_pos()}")
    #
    # print("getl", robot.robot.getl())
    #
    # robot.set_gripper_position(100)
    # print("Command Gripper Pos: 100")
    # print(f"Read Gripper Pos: {robot.get_gripper_pos()}")
    #
    # robot.set_gripper_position(0)
    # print("Command Gripper Pos: 0")
    # start = time.time()
    # print(f"Read Gripper Pos: {robot.get_gripper_pos()}")
    # print(time.time() - start)
    #
    # print(robot.get_robot_state())
    # # robot.robot.movel((-0.2, -0.45, 0.14, -2.2213, 2.2213, 0), acc=0.5, vel=2.5)
    # # robot.robot.movel((-0.2, -0.45, 0.10, -2.2213, 2.2213, 0), acc=0.5, vel=2.5)
    # # robot.robot.movel((-0.2, -0.45, 0.14, -2.2213, 2.2213, 0), acc=0.5, vel=2.5)
    # del robot
